...
function generarTresSemanas($conn) {
    ...
    foreach ($domingos as $domingo) {
        ...
        $mes_numero = (int)$viernes->format('n');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'actualizar') {
    if (!validarCSRF($_POST['csrf_token'])) {
        ...
        $_SESSION['mensaje'] = "✅ Sistema actualizado. 3 semanas activas.";
        if ($semanas_desactivadas > 0) {
            $_SESSION['mensaje'] .= " Se desactivaron $semanas_desactivadas semanas antiguas.";
        }
